...
const { YagoyaCommandService } = await import("../js/services/command-service.js");
...
function fresh() {
  ...
  const commands = new YagoyaCommandService({ store, paymentService, telemetry });
...
test("Owner/Admin authority is distinct and Yagoya cannot lose its last Owner", () => {
  const { store, commands } = fresh();
...
test("Yagoya Admin can onboard and administer drivers with an audit trail", () => {
  const { store, commands } = fresh();
  const admin = store.platformActor("admin");
...
test("Yagoya Admin all-order oversight stays bounded and searchable", () => {
  const { store, repos } = fresh();
